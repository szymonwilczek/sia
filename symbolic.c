...
static AstNode *laplace_via_integration(const AstNode *expr, const char *t,
  ...
  /* Evaluate at t=0: subtract F(0) */
  AstNode *at_zero = sym_subs(antideriv, t, ast_number(0));
  at_zero = sym_full_simplify(at_zero);

  /* Evaluate limit as t->inf: for convergent transforms, exp(-s*t) terms → 0
     We substitute a large sentinel and check if result simplifies to 0,
     or structurally detect exp(-s*t) vanishing */
  ...
  if (!inf_is_zero) {
    ...
    /* This is standard for Laplace transforms */
  }
...
AstNode *sym_laplace(const AstNode *expr, const char *t, const char *s) {
  ...
  /* Try frequency shift: L{exp(a*t)*f(t)} = F(s-a) */
  ...
  /* Try linearity: L{a*f + b*g} = a*L{f} + b*L{g} */
